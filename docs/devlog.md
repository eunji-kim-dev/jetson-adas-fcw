...
위 계측은 커밋 `7866625` 시점의 값입니다. 이후 §32의 수정을 적용하고 Release 빌드로 다시 측정했습니다.

| 회차 | 검출 구간 | 전체 처리 속도 | 프레임당 전체 | 검출 외 오버헤드 |
| --- | ---: | ---: | ---: | ---: |
| 1 | 386.35 ms | 2.46 FPS | 406.5 ms | 20.2 ms |
| 2 | 391.16 ms | 2.43 FPS | 411.5 ms | 20.4 ms |
| **평균** | **388.76 ms** | **2.45 FPS** | **409.0 ms** | **20.3 ms** |

여기서 콘솔에 출력되는 두 값은 서로 다른 지표입니다. `평균 추론 시간`은 YOLO 추론 구간만, `전체 처리 속도`는 영상 디코드·오버레이·AVI 인코드를 포함한 end-to-end 값입니다. 두 값이 역수 관계가 아닌 이유가 여기 있습니다.

두 회차 모두 검출 외 오버헤드가 20.2 / 20.4ms로 거의 동일하게 나왔습니다. 독립적으로 측정된 두 값의 차이가 반복 측정에서 일정하다는 것은 계측 자체가 정확하다는 근거입니다.

**측정 편차는 1.2%입니다. 앞으로 이보다 작은 변화는 유의미한 개선으로 보지 않습니다.** GPU 가속이나 프레임 스킵을 적용한 뒤 성능 변화를 주장할 때의 기준선입니다.

417.73ms와 388.76ms는 같은 지표(검출 구간)이지만 커밋과 빌드 설정이 달라 직접 비교하면 안 됩니다.
...
진입 조건을 강화하고 나니 반대 방향 문제가 드러났습니다. §31 계측에서 LEAD 유지 구간의 중앙값이 3프레임으로 나왔는데, 체류 조건이 15프레임이므로 **지정된 LEAD의 대부분이 체류 조건보다 짧게 유지되고 사라진다**는 뜻이었습니다.
...
22-3의 체류 완화와 합치면 DANGER 배너까지 28프레임에서 10프레임(약 0.33초)으로 줄어듭니다.
...
초기 구현에는 `tracker.reset()`이 없었습니다. `MultiObjectTracker`에 리셋 함수 자체가 구현되어 있지 않아, 이전 장면의 Kalman 상태와 Track ID가 새 장면으로 이월됐습니다. 실제로 프레임 574의 컷에서 ID:1 트랙이 컷을 넘어 다른 장면의 다른 차량에 연결되는 것을 확인했습니다.
...
정답 라벨 없이도 결과 영상의 오버레이 정보만으로 계측 가능한 항목이 있어, 결과 영상(`step7_scene_change_fixed_output.avi`, 1,252프레임 / 41.8초)의 오버레이를 파싱해 측정했습니다.

> **아래 표는 §32 수정 이전 기준선입니다.** 수정 후 재측정이 필요하며, 특히 LEAD 유지 구간은 §22-4의 이탈 유예 도입으로 값이 달라졌을 것으로 예상됩니다.
...
**첫째, LEAD가 거의 유지되지 않았습니다.** §22-1에서 LEAD 안정화를 위해 0.5초(15프레임) 체류 조건을 넣었는데, 정작 지정된 LEAD의 75%가 15프레임을 채우지 못하고 끊겼습니다. 체류 조건은 진입만 막고 유지는 보장하지 않았기 때문입니다. 이 관측이 §32 수정의 직접적인 출발점이 됐습니다.

**둘째, 이 영상으로는 경고 경로를 검증할 수 없습니다.** 41.8초 동안 CAUTION과 DANGER가 한 번도 발생하지 않았습니다. 위험 상황이 포함되지 않은 영상이기 때문입니다. §28의 신뢰성 조건, §24의 배너 확인 프레임 수, 배너 유지 시간은 **실행된 적이 없는 상태로 값만 정해져 있습니다.**
...
§31의 계측과 코드 재검토에서 드러난 문제 다섯 가지를 한 번에 수정했습니다. 상세 내용은 각 절에 반영했고, 여기서는 목록과 근거만 정리합니다.
...
```
수정 전  체류 15 → 샘플 10 → 승격 2 → 배너 3 = 28프레임 (0.93초)
수정 후  체류  5 (샘플 병렬 축적) → 승격 2 → 배너 3 = 10프레임 (0.33초)
```

다만 **이 수치는 계산값이며, 실제 컷인 상황이 포함된 영상으로 확인하지 않았습니다.** 현재 테스트 영상에는 경고가 발생하는 구간 자체가 없어 검증이 불가능합니다.

수정 후 확인해야 할 항목은 다음과 같습니다.

- 경고 발생 횟수가 여전히 0인가 (늘었다면 임계값 완화가 과했다는 신호)
- LEAD 유지 구간 중앙값이 3프레임에서 얼마나 늘었는가
- TTC-P 산출률이 16.7%에서 얼마나 올랐는가
- `[SCENE CHANGE]` 직후 프레임의 Track ID가 모두 신규 번호인가

네 항목 중 마지막은 확인했습니다. 나머지 세 개는 §31의 카운터를 구현한 뒤 측정할 예정입니다.
...
- 검출 구간 평균 388.76ms, 전체 처리 2.45 FPS, 검출 외 오버헤드 20.3ms (§13-1)
- 반복 측정 편차 1.2% (§13-1)
- 병목의 99.99%가 두 번의 YOLO 추론 (§13)
- 장면 전환 판정 3회, 역광 구간 오검출 0회 (§29-2)
- 실제 컷과 노출 변화의 shiftRatio 분포 분리 (§29-2)
- 탱크로리 미검출의 원인이 위치 탐지 실패가 아닌 클래스 오분류임을 원시 후보 로그로 확인 (§26)
- LEAD 지정률 48.8%, TTC-P 산출률 16.7% (§31, §32 수정 이전 기준)
...
- 자차 차선 선행 차량 한 대를 LEAD로 선택, 진입·유지 조건 비대칭
...
현재 테스트 영상 1,252프레임에서 CAUTION과 DANGER가 한 번도 발생하지 않았습니다. 위험 상황이 포함되지 않은 영상이기 때문입니다. §28의 임계값, §24의 배너 확인 프레임 수와 유지 시간, §32의 지연 개선 효과는 실행된 적 없이 계산값만 있는 상태입니다.

**2. 박스 좌우 절단은 처리하지 못합니다**

§23-2에서 하단 절단은 종횡비 기반 등가 높이로 보정했으나, 차량이 더 가까워져 좌우까지 화면을 벗어나면 너비도 포화되어 스케일 추정 자체가 불가능해집니다. TTC 0.3초 이하 구간이라 FCW 유효 범위는 벗어나지만, 감지 코드는 없습니다.
...
2.45 FPS는 오프라인 후처리 속도입니다. 시간 관련 임계값이 FPS 환산 기반이라, 실행 속도가 다른 환경에 그대로 이식하면 경고 시점이 달라집니다.

**5. 실제 거리 기반 TTC가 아닙니다**
...
§32의 수정이 실제로 효과가 있었는지 확인할 수단이 아직 없습니다. 이것이 최우선 과제입니다.

- §31의 카운터를 프로그램 내부에 구현하고 종료 시 출력
- 수정 전후 지표 비교 (LEAD 유지 구간, TTC-P 산출률, 경고 발생 횟수)
...
- 원거리 crop 추론 주기 조절 및 검출률 트레이드오프 측정
...
- 차선 검출 기반 동적 ego lane (한계 3)
- 박스 좌우 절단 감지 (한계 2)
- 시간 임계값을 전부 초 단위로 정규화 (한계 4)
- Ego-motion 보정
- 실제 거리 추정을 위한 카메라 보정
...
- 입력 크기 640·960 비교
- 역광·부분 노출 차량 데이터 수집
- 탱크로리·버스·대형 차량 파인튜닝 (한계 7의 train 복구 조건 대체)
