...
좌표는 고정 픽셀 대신 영상 너비와 높이의 비율로 계산했습니다.
...
영상 밖으로 나가는 좌표는 `std::clamp()`로 잘랐고, 복원한 뒤 너비나 높이가 0 이하가 된 박스는 버렸습니다.
...
각 그룹별로 `NMSBoxes()`를 따로 돌린 뒤 결과를 하나의 Detection 목록으로 합쳤습니다. 이후 같은 차량에 서로 다른 클래스 박스가 겹쳐 남아 별도 Track이 생기는 문제가 줄었습니다.
...
여기서 추적 안정성을 더 높이기 위해 검출 하한선을 0.10까지 내리고 Detection을 두 등급으로 나눴습니다.
...
저신뢰도 후보를 완전히 버리지는 않되, 그것으로 새 ID가 만들어져 오검출이 새 차량으로 둔갑하는 것은 막기 위해 이렇게 나눴습니다.
...
## 10. 프레임별 추론 시간 측정과 병목 분석

YOLO 추론 시작과 종료 시각을 `std::chrono::steady_clock`으로 측정해 프레임별 지연시간을 기록했습니다.

초기에는 전체 검출 구간만 하나의 값으로 측정했지만, 원거리 차량 검출을 보완하기 위해 중앙 도로 영역을 crop한 뒤 같은 YOLO 모델로 한 번 더 추론하는 구조가 추가되면서 처리 시간이 크게 증가했습니다.
...
현재 환경은 OpenCV DNN의 CPU 추론이며, 평균 검출 속도는 약 **2.39 FPS**였습니다.
...
판정 방식은 다음과 같이 바꿨습니다. IoU가 충분히 높으면 바로 동일 객체 후보로 두고, IoU가 낮더라도 중심 거리가 허용 범위 안이면 추가 조건을 확인한 뒤 동일 객체 후보로 인정했습니다.
...
그래서 초기 Track에는 완화된 조건을 적용했습니다. hits가 적은 초기 Track은 중심 거리와 면적 변화 허용 범위를 넓혀주고, 최소 2~3프레임 동안은 속도를 학습할 기회를 줬습니다. 이후 같은 차량에 매 프레임 새 ID가 생기는 현상이 크게 줄었습니다.
...
그 외에 중심점 거리 제한, 박스 면적 변화 제한, 복구 가능한 누락 프레임 수 제한, 같은 클래스 그룹 가산점, 오래 놓친 Track에 대한 추가 감점 같은 조건도 함께 걸었습니다. 이후 정상적으로 유지되던 ID를 과거 ID가 가로채는 경우가 줄었습니다.
...
상태값을 6개에서 8개로 늘렸습니다. 중심 x, 중심 y, x 방향 속도, y 방향 속도, 박스 너비, 박스 높이, 너비 변화속도, 높이 변화속도입니다. 박스 크기 변화속도까지 상태에 넣어, 차량이 접근하거나 멀어질 때 다음 프레임의 크기 변화도 예측에 반영되도록 했습니다.
...
ByteTrack 방식을 참고해 Detection을 두 단계로 나눠 썼습니다.
...
옆 차선 차량은 계속 검출하고 추적하되, 위험 분석 대상에서만 제외했습니다. ego lane 내부 차량은 선행 차량 후보로 두고, driving ROI 안이지만 ego lane 밖이면 SIDE 객체로 분류했습니다. 이 변경으로 측면의 대형 트럭이나 인접 차선 차량이 위험 경고를 일으키는 문제가 줄었습니다.

## 18. 선행 차량 LEAD 선택

ego lane 안에 차량이 여러 대 있을 수 있어서, 위험 분석 대상 한 대를 고르는 로직을 추가했습니다. 각 후보에 대해 ego lane 내부 여부, 박스 하단 중앙점 위치, 화면 아래쪽에 얼마나 가까운지, 차선 중심선과 얼마나 가까운지, 현재 Track ID의 연속성을 확인했습니다. 화면 아래쪽에 있고 차선 중앙에 가까운 차량일수록 높은 점수를 받습니다.

프레임마다 후보가 조금씩 흔들려 LEAD ID가 계속 바뀌는 것을 막기 위해 히스테리시스를 넣었습니다. 기존 LEAD가 계속 보이면 기본적으로 유지하고, 새 후보가 나타나도 기존 후보보다 점수가 충분히 높을 때만 교체합니다. 위험 분석 대상이 한 대로 좁혀지면서 화면 정보도 단순해지고 경고 기준도 더 명확해졌습니다.
...
선행 차량의 바운딩 박스 높이가 시간에 따라 얼마나 빠르게 커지는지를 지표로 사용했습니다.
...
한두 프레임의 박스 흔들림에 결과가 휘둘리지 않도록, 최근 여러 샘플에 선형회귀를 적용했습니다. `RiskAnalyzer`가 관리하는 주요 정보는 프레임 번호, 박스 높이, 박스 하단 y좌표, 최근 샘플 수, log-height 회귀 기울기, 하단점 y방향 이동 속도, 박스 높이 증가 비율, TTC-P, 위험 단계입니다.
...
이 상태를 그대로 화면에 내보내면 경고가 계속 깜빡입니다. 그래서 `RiskAnalyzer` 안에 안정화 로직을 넣었습니다. 새 위험 단계가 일정 프레임 이상 유지돼야 단계 변경을 확정하고, 위험 단계가 낮아지는 방향이면 즉시 낮추지 않고 일정 시간 더 확인합니다. Track이 LEAD에서 빠지면 그 Track의 위험 이력은 초기화합니다. 상단 경고 배너도 일정 시간 유지해서 한두 프레임 누락만으로 바로 사라지지 않게 했습니다.

## 21. 경고 단계와 TTC-P 숫자 표시 분리

위험 단계는 안정화 때문에 잠시 유지되는데, TTC-P 숫자는 매 프레임 갱신됩니다. 이 때문에 다음처럼 모순되어 보이는 표시가 나왔습니다.
...
현재 TTC-P는 선행 차량 라벨과 왼쪽 상태 정보에서만 보여주기로 했습니다. 상단 배너는 현재 위험 단계를, 선행 차량 라벨과 상태창은 현재 TTC-P 수치를 각각 맡도록 역할을 나누고 나니, 경고와 수치가 서로 모순처럼 보이는 문제는 사라졌습니다.
...
그런데 처리 시간만 늘고 검출 시점은 거의 나아지지 않았습니다. 원인을 정확히 확인하기 위해 18~24초 구간의 필터링 전 YOLO 후보를 CSV로 저장했습니다. frame, time, 상위 1·2·3순위 클래스, car / bus / train / truck / boat 점수, 박스 좌표와 크기, confidence 통과 여부, 대상 클래스 통과 여부, 최종 필터 통과 여부까지 기록했습니다.
...
즉 검출 위치를 찾지 못한 것이 아니라, 클래스 오분류와 후처리 필터의 문제였습니다.
...
모든 train을 truck으로 바꾸면 다른 장면에서 오검출이 늘어날 위험이 있어 조건을 제한했습니다. 원래 최고 클래스가 train이고, 신뢰도 0.25 이상이고, 화면 오른쪽에 위치하고, 박스가 충분히 크고, 박스 하단이 도로 영역까지 내려오는 경우에만 truck(7)으로 재해석해 기존 차량 NMS와 Tracker에 넘기도록 했습니다.
...
멀리 있는 작은 선행 차량은 바운딩 박스 높이가 몇 픽셀만 바뀌어도 비율상 큰 변화로 계산됩니다. 그래서 실제로는 급접근이 아닌데 TTC-P가 순간적으로 작아지면서 CAUTION이 뜨는 문제가 있었습니다.

TTC-P 임계값 하나만 보지 않고 다음 조건을 함께 확인하도록 했습니다.
...
이 조건을 통과하지 못한 위험 결과는 내부 계산값이 있더라도 화면에서는 SAFE로 처리했습니다. 이후 작은 원거리 박스의 검출 흔들림 때문에 생기던 오경고가 줄었습니다.
...
장면 전환이 감지되면 상단 경고 유지 카운터, CAUTION/DANGER 후보 프레임 수, 현재 LEAD ID, TTC-P 분석 상태를 전부 초기화합니다. 새 장면 직후에는 약 0.5초 동안 위험 분석을 멈춰서, 검출과 추적이 안정화된 뒤에 LEAD를 다시 고르도록 했습니다.

상단 배너도 위험 상태가 연속으로 관찰될 때만 뜨도록 확인 프레임을 뒀습니다(CAUTION은 8프레임, DANGER는 3프레임 연속 확인). 이후 차량이 없는 새 장면에 이전 장면의 경고가 잠깐 남는 현상이 사라졌습니다.
...
Track 보관 시간보다 오래 YOLO 검출이 끊기면 새 ID가 생성됩니다. 현재 추적기는 위치와 크기 중심이라 이전 차량의 색상이나 외형을 비교할 수 없습니다. 장시간 동일 차량 여부를 판단하려면 Re-ID 특징이 필요합니다.

**3. 부분 노출 객체 조기 검출 한계**

오른쪽 탱크로리는 21.9초 이후의 train 오분류는 복구할 수 있었지만, 19~21초에는 의미 있는 YOLO 후보 자체가 거의 없었습니다. 이 구간을 해결하려면 YOLOv8s 이상 모델 사용, 입력 해상도 확대, 부분 노출·역광 탱크로리 데이터 파인튜닝, 전역 카메라 모션 보정, 우측 경계 이동 객체 감지, Edge 후보와 정식 YOLO Track 연결 같은 방법을 검토해야 합니다.

**4. 특정 테스트 영상에 맞춘 Rule-based 조건**

train 오분류 복구 조건과 장면 전환 임계값은 현재 테스트 영상의 특성을 기반으로 조정한 값입니다. 다양한 영상에 적용하려면 여러 도로 환경에서 정량 평가와 재조정이 필요합니다.
