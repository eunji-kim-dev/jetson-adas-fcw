...
| **전체 검출 구간** | **355.05 ms** | **100%** |

측정 전에는 Detection 병합이나 그룹 NMS, 중복 박스 제거 같은 후처리도 처리시간 증가에 어느 정도 영향을 줄 것으로 예상했습니다. 하지만 실제 측정에서는 후처리가 평균 0.022ms에 불과했고, 전체 지연시간의 거의 전부가 두 번의 YOLO 추론에서 발생하고 있었습니다.

특히 원거리 crop은 원본 영상보다 작은 영역이지만, crop 이미지 역시 `detectObjects()`에 전달된 뒤 다시 640×640 입력으로 변환되어 동일한 YOLOv8n 네트워크 전체를 통과합니다. 그래서 전체 프레임 추론과 원거리 crop 추론의 평균 시간이 178.70ms와 176.33ms로 거의 동일하게 측정됐습니다.
...
```
평균 추론 시간  : 355.05 ms
추론 기준 FPS  : 2.82 FPS     ← YOLO 추론 구간만
전체 처리 속도  : 2.67 FPS     ← 디코드 · 오버레이 · AVI 인코드 포함
```

역산하면 다음과 같습니다.

```
1 / 2.82 = 354.6 ms   (추론, 355.05와 일치)
1 / 2.67 = 374.5 ms   (전체)
374.5 − 355.05 = 19.5 ms   (검출 외 오버헤드)
```
...
평균 하나로는 실시간성을 판단할 수 없습니다. 한 프레임이라도 크게 밀리면 그 프레임의 위험 판정과 경고가 함께 늦어지기 때문입니다. 1,252프레임 전체의 분포를 계측했습니다.
...
**최소 지연조차 141ms이므로 모든 프레임이 목표를 초과했습니다.** 최종 목표는 평균 66.7ms 달성이 아니라 **P95를 66.7ms 이하로 낮추고 Deadline Miss Rate를 최소화하는 것**으로 잡았습니다. 최대값은 OS 스케줄링 같은 단발성 요인이 섞이므로 주 지표가 아니라 참고값으로 봅니다.
...
앞으로 성능 변화를 주장할 때는 10%를 넘는 차이만 유의미한 것으로 봅니다. Jetson 이식 후에도 동일한 방식으로 30분 연속 구동 시 thermal throttling에 따른 지연 추이를 측정할 계획입니다.
...
```
[PERF] frame=1252 | Full YOLO=155.66 ms | Far YOLO=173.81 ms | Post=0.02 ms | Total=329.50 ms | AVG=355.05 ms
```
...
이 정비 덕분에 §13-2의 지연 분포(P95 / P99 / Deadline Miss Rate)와 §13-3의 구간별 성능 추이를 로그만으로 산출할 수 있게 됐습니다.
...
- 검출 구간 평균 355.05ms, 중앙값 334.16ms, P95 563.96ms, P99 701.59ms, 최대 849.06ms (§13-2)
- 추론 기준 2.82 FPS, 전체 파이프라인 2.67 FPS, 검출 외 오버헤드 19.5ms (§13-1)
- 15 FPS(66.7ms) 기준 Deadline Miss Rate 100%, 최소 지연조차 141.21ms (§13-2)
- 병목의 99.99%가 두 번의 YOLO 추론, 후처리는 0.022ms (§13)
- 4회 반복 실행 편차 9.6%, 원인은 실행 중 CPU 클럭 하강 (§13-3)
...
15 FPS 목표(66.7ms) 대비 P95가 8.5배, Deadline Miss Rate 100%입니다. 시간 관련 임계값이 FPS 환산 기반이라, 실행 속도가 다른 환경에 그대로 이식하면 경고 시점이 달라집니다.
...
- 30분 연속 구동 시 thermal throttling에 따른 지연 추이 (§13-3과 동일 방식)
