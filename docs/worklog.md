...
### 이 작업을 한 이유
- `RiskAnalyzer`만 `adas`로 옮기고 LEAD 선정 코드가 `main.cpp`에 남아 있으면 FCW 판단 로직이 아직 완전히 분리된 것이 아니기 때문
- `main.cpp`는 전체 기능을 연결하는 역할만 하고, 선행 차량 선정과 위험 판단은 `adas`가 맡도록 구조를 정리하기 위해
- `perception`의 추적 객체에는 LEAD 여부나 위험도 같은 FCW 전용 정보를 넣지 않고, 일반적인 추적 정보만 남기기 위해