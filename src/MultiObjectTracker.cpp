...
MultiObjectTracker::TrackState::TrackState(
    ...
    /*
     * YOLO에서 직접 얻는 측정값:
     * 중심 x, 중심 y, 박스 너비, 박스 높이
     */
    kalman.measurementMatrix =
        cv::Mat::zeros(4, 8, CV_32F);
    ...
    /*
     * 위치보다 속도와 크기 변화속도의 불확실성을 크게 설정합니다.
     *
     * 블랙박스 영상은 카메라 자체가 이동하기 때문에
     * 객체 중심과 박스 크기가 빠르게 달라질 수 있습니다.
     */
    cv::setIdentity(
        kalman.processNoiseCov,
        cv::Scalar::all(1.0e-2)
    );
    ...
    const float centerX =
        detection.box.x +
        detection.box.width / 2.0F;

    const float centerY =
        detection.box.y +
        detection.box.height / 2.0F;
...
MultiObjectTracker::MultiObjectTracker(
    ...
    : highConfidenceThreshold_(
          highConfidenceThreshold
      ),
      lowConfidenceThreshold_(
          lowConfidenceThreshold
      ),
      minConfirmationHits_(
          minConfirmationHits
      ),
      maxMissedFrames_(
          maxMissedFrames
...
float MultiObjectTracker::calculateAllowedCenterDistance(
    ...
    /*
     * 기존 상한 100px은 가까운 대형 차량에 너무 작았습니다.
     *
     * 박스 대각선에 비례해 허용 거리를 늘리고
     * 다른 차량까지 연결되는 것을 막기 위해
     * 최대값은 320px로 설정합니다.
     */
    return std::clamp(
...
cv::Rect2f MultiObjectTracker::stateToRect(
    const cv::Mat& state
) {
    const float centerX =
        state.at<float>(0);
...
cv::Mat MultiObjectTracker::detectionToMeasurement(
    const Detection& detection
) {
    const float centerX =
        detection.box.x +
...
bool MultiObjectTracker::isSameTrackingCategory(
    int firstClassId,
    int secondClassId
) {
    const auto isFourWheelVehicle =
        [](int classId) {
            return classId == 2 ||
                   classId == 5 ||
                   classId == 7;
    ...
    if (
        isFourWheelVehicle(firstClassId) &&
        isFourWheelVehicle(secondClassId)
    ) {
        return true;
    }

    if (
        isTwoWheelVehicle(firstClassId) &&
        isTwoWheelVehicle(secondClassId)
    ) {
        return true;
    }
...
std::vector<TrackedObject> MultiObjectTracker::update(
    ...
    for (int detectionIndex = 0;
        ...
        const float confidence =
            detections[detectionIndex].confidence;

        if (
            confidence >=
            highConfidenceThreshold_
        ) {
            highDetectionIndices.push_back(
                detectionIndex
            );
        } else if (
            confidence >=
    ...
    /*
     * 기존 Track의 다음 위치와 크기를 예측합니다.
     */
    for (TrackState& track : tracks_) {
        const cv::Mat predictedState =
            track.kalman.predict();

        track.box =
            stateToRect(predictedState);

        ++track.age;
        ++track.missedFrames;
    }

    const int previousTrackCount =
    ...
    const auto performMatching =
        [&](
            ...
            for (const int trackIndex
                ...
                /*
                 * 초기 Track은 아직 속도를 충분히 학습하지 못했습니다.
                 *
                 * confirmed가 아니거나 hits가 확인 프레임 수보다
                 * 작으면 초기 Track으로 판단합니다.
                 */
                ...
                for (const int detectionIndex
                    ...
                    const float allowedDistance =
                        calculateAllowedCenterDistance(
                            track.box,
                            detectionBox
                        );
                    ...
                    if (
                        matchPass ==
                        MatchPass::RecentHigh
                    ) {
                        /*
                         * 새 Track은 속도 0으로 시작하므로
                         * 처음 2~3회의 매칭에서는 거리 게이트를
                         * 넓게 적용합니다.
                         */
                        if (immatureTrack) {
                            iouGate = 0.01F;
                        ...
                        /*
                         * 저신뢰도 검출은 기존 Track 유지용입니다.
                         *
                         * 초기 Track도 재시도 기회를 받지만
                         * 고신뢰도 검출보다 조금 엄격하게 판단합니다.
                         */
                        iouGate = 0.03F;
                        ...
                        /*
                         * 여러 프레임 놓친 확정 Track입니다.
                         *
                         * 놓친 프레임 수가 늘수록 이동 가능한 거리도
                         * 늘어나므로 허용 거리를 단계적으로 확장합니다.
                         */
                        iouGate = 0.03F;
                     ...
                     * 거리 조건이 해당 후보를 다시 거부하지 않습니다.
                     *
                     * 거리 게이트는 IoU가 낮을 때만
                     * 보조 수단으로 사용합니다.
                     */
                    const bool passedByIoU =
                        iou >= iouGate;
                    ...
                    /*
                     * 박스 면적 차이도 IoU가 낮은 경우에만
                     * 하드 게이트로 사용합니다.
                     *
                     * IoU가 충분히 높다면 거리나 면적 조건이
                     * 마지막에 다시 매칭을 뒤집지 않습니다.
                     */
                    if (
                        !passedByIoU &&
                        areaRatio >
                    ...
                    /*
                     * 여러 프레임 놓친 과거 ID에는 감점을 줍니다.
                     *
                     * 정상적으로 이어지고 있는 ID가
                     * 오래된 ID보다 우선 연결되도록 합니다.
                     */
                    if (
                        matchPass ==
                        MatchPass::RecoveryHigh
                    ) {
                        cost +=
                            static_cast<float>(
                                track.missedFrames -
                    ...
                    /*
                     * 같은 차량이 car와 truck 사이에서
                     * 분류가 바뀌는 것은 허용하지만,
                     * 완전히 같은 classId인 후보를 조금 우선합니다.
                     */
                    if (
                        track.classId !=
                        detection.classId
            ...
            for (const MatchCandidate& candidate
                 : candidates) {
                if (
                    matchedTracks[
                ...
                const bool lowConfidenceMatch =
                    matchPass ==
                    MatchPass::RecentLow;

                /*
                 * 저신뢰도 결과는 Track을 유지하는 데 사용하지만,
                 * 미확정 Track을 정식 ID로 승격시키는 근거로는
                 * 사용하지 않습니다.
                 */
    ...
    /*
     * predict() 후 missedFrames == 1이면
     * 직전 프레임까지 검출됐던 Track입니다.
     *
     * 확정·미확정 Track을 모두 포함합니다.
     */
    ...
    /*
     * 1차:
     * 최근 Track과 고신뢰도 검출을 연결합니다.
     *
     * 초기 Track은 이 단계에서 완화된 거리 조건을 사용합니다.
     */
    performMatching(
        recentTrackIndices,
    ...
    /*
     * 2차:
     * 1차에서 실패한 missedFrames == 1 Track을
     * 저신뢰도 검출과 다시 연결합니다.
     *
     * 기존 코드와 달리 확정 Track뿐 아니라
     * 미확정 초기 Track도 재시도 기회를 받습니다.
     */
    ...
    for (const int trackIndex
         : recentTrackIndices) {
        if (!matchedTracks[trackIndex]) {
            unmatchedRecentTracks.push_back(
                trackIndex
            );
        }
    }
    ...
    /*
     * 3차:
     * 2프레임 이상 놓친 확정 Track을
     * 남은 고신뢰도 검출과 재연결합니다.
     */
    std::vector<int> lostConfirmedTracks;

    const int maximumRecoveryFrames =
        std::min(
            maxMissedFrames_,
            10
        );
...
// 0.26짜리 순간 오검출이 새 ID로 인식되는 것을 방지
    ...
    /*
     * 미확정 Track은 두 프레임 연속 놓치면 삭제합니다.
     *
     * 확정 Track은 maxMissedFrames까지 상태를 보관합니다.
     */
    tracks_.erase(
        std::remove_if(
            tracks_.begin(),
